...
export default function App() {
  ...
  const handleSessionComplete = (mode: AppMode) => {
    ...
    if (mode === "focus") {
      ...
      notify('success', 'عاش! انتهت جلسة التركيز بنجاح واكتسبت نقطة 💚');
      
      // Auto-remove task after focus session
      setActiveTask(null);
    } else if (mode === "break") {
      notify('info', 'انتهى وقت الراحة، يلا نرجع للتركيز؟ 💪');
    } else {
      notify('info', 'اكتملت جلسة النوم، نتمنى لك استيقاظًا نشيطًا 🌅');
    }
  };
